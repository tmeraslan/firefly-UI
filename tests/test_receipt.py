...
class TestReceiptInboxParse(SETUPTEST):
    ...
    def test_receipt_inbox_parse_image(self):
        ...
        try:
            ...
            # ✅ Save the merchant **before** creating the transaction
            ...
            # ✅ Verify feedback message (not expecting ID inside the message)
            alert_text = receipt.get_alert_text()
            self.assertTrue(alert_text)

            # ✅ Go back to the Receipt Inbox page so the search form is present
            home.go_to_receipt_inbox()

            # ✅ Search by the merchant we saved
            ...
            # Extract the Receipt ID from results and then search by ID
